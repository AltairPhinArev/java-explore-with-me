...
import ru.practicum.ewmmainservice.errorhandling.exceptions.*;
...
public class ErrorHandler {
    ...
    @ExceptionHandler
    @ResponseStatus(HttpStatus.CONFLICT)
    public ApiError handleForbiddenException(final ConditionsNotMeetException e) {
        log.error(e.getLocalizedMessage(), e.getMessage());
        return new ApiError(
                HttpStatus.CONFLICT.toString(),
    ...
    @ExceptionHandler
    @ResponseStatus(HttpStatus.BAD_REQUEST)
    public ApiError handleonstraintViolationException(final ValidationException e) {
        log.error(e.getLocalizedMessage(), e.getMessage());
